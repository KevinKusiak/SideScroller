...
//  IndieType
//
//  Created by conantcougars on 1/27/20.
//  Copyright © 2020 KevinKusiak. All rights reserved.
//
...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let character = SKSpriteNode(imageNamed: "RPGtest")
        character.position = CGPoint(x: -240, y: -80)
        
        addChild(character)
        
        let moveBottomLeft = SKAction.move(to: CGPoint(x: 0,y: 0), duration:2.0)
        character.run(moveBottomLeft)
        ...
        let sequence2 = SKAction.sequence([moveRight])
        character.run(sequence2)
        
        
//        let endlessAction = SKAction.repeatForever(sequence)
//        character.run(endlessAction)
